...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b-2 border-blue-600 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <nav className="flex items-center justify-between">
              ...
              {navItems.map((item) => (
                <li key={item.label}>
                  <a
                    ...
                    className="flex items-center gap-1 text-slate-700 hover:text-blue-600 transition-colors font-semibold text-base cursor-pointer"
                  >
                    {item.label}
            ...
            <div className="flex items-center gap-3">
              <Button 
                variant="outline" 
                className="hidden sm:inline-flex border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white font-semibold"
                onClick={handleConnexion}
              >
                Connexion
              </Button>
              <Button 
                className="hidden sm:inline-flex bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-sm"
          ...
          {/* Mobile Menu */}
          {mobileMenuOpen && (
            <div className="lg:hidden mt-4 pb-4 space-y-4 border-t border-slate-200 pt-4">
                ...
                {navItems.map((item) => (
                  <li key={item.label}>
                    <a
                      href={item.href || "#"}
                      onClick={(e) => handleNavClick(e, item.href || "#", item.onClick)}
                      className="flex items-center gap-1 text-slate-700 hover:text-blue-600 transition-colors font-semibold py-2 cursor-pointer"
                    >
                      {item.label}
                      {item.hasDropdown && <ChevronDown className="h-4 w-4" />}
                    </a>
              ...
              <div className="flex flex-col gap-2 pt-4 border-t border-slate-200">
                <Button variant="outline" className="w-full border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white" onClick={handleConnexion}>
                  Connexion
                </Button>
                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white" onClick={handleMonEspace}>Mon espace</Button>
              </div>
            </div>
          )}
        </div>
